...
from multidocchat.utils.document_ops import FastAPIFileAdapter,read_pdf_via_handler
...
print (BASE_DIR)
p = app.mount("/static", StaticFiles(directory=str(BASE_DIR/"static")), name ="static")
print(p)
templates =Jinja2Templates(directory= str(BASE_DIR /"templates"))

app.add_middleware(CORSMiddleware,
    allow_origins=["*"],
    # allow_credentials=True, <-- was causing conflict
    allow_methods=["*"],
    allow_headers=["*"],
    )
...
async def chat_build_index(
    ...
    try:
        ...
        log.info(f"Index created succesfully for session : {ci.session_id}")
...
async def chat_query(
    ...
    try:
        log.info(f"Reciever chat query: '{question}' | session: {session_id}")
