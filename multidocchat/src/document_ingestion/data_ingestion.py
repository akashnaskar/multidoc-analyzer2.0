...
from multidocchat.utils.file_io import generate_session_id, save_uploaded_files
from multidocchat.utils.document_ops import load_documents, concat_for_analysis, concat_for_comparison
...
class FaissManager:
    def __init__(self, index_dir: Path, model_loader: Optional[ModelLoader]=None):
        ...
        self._meta : Dict[str, Any]={"rows":{}} #thisd is dict of rows

        if self.meta_path.exists():
            try:
                self._meta= json.loads(self.meta_path.read_text(encoding="utf-8")) or {"rows":{}}#load if iots already there
            except Exception as e:
                self._meta= {"rows":{}} #init if the empty oner does not exist
...
class ChatIngestor:
    def __init__(self,
        ...
        try:
            ...
            log.info("ChatIngestor initialization",
    ...
    def built_retriever(self,
        ...
        try:
            ...
            #FAISS manager very very imp class for the docchat
...
class DocHandler:
    ...
    def _save_file(self, uploaded_file)-> str:
        try:
            filename = os.path.basename(uploaded_file.name)
            ext= os.path.splitext(filename)[1].lower()
            allowed={".pdf", ".docx", ".pptx", ".md", ".txt", ".xlsx", ".xls", ".csv", ".db", ".sqlite", ".sqlite3"}
            if ext not in allowed:
                raise ValueError(f"Unsupported file type: {ext}. Aloowed: {sorted(allowed)}")
            ...
            log.info("File saved succesfully", file= filename, save_path=save_path, session_id= self.session_id)
            return save_path
        except Exception as e:
        ...
        try:
            ...
            with open(save_path, "wb") as f:
                if hasattr(uploaded_file, "read"):
                    ...
                    f.write(uploaded_file.getbuffer())
            log.info("PDF saved succesfully", file= filename, save_path=save_path, session_id = self.session_id)
            return save_path
        except Exception as e:
    ...
    def read_pdf(self, pdf_path: str)-> str:
        try:
            ...
            texts = "\n".join(text_chunks)
            log.info("PDF read succesfully", pdf_path = pdf_path, session_id= self.session_id, pages= len(text_chunks))
            return texts
        except Exception as e:
